...
export default function AddIdeaModal() {
  ...
  const [ideaTags, setIdeaTags] = useState<{ [category: string]: string[] }>({});
  ...
  const toggleTag = (category: string, tag: string) => {
    const currentTags = ideaTags[category] || [];
    const updatedTags = currentTags.includes(tag)
      ? currentTags.filter(t => t !== tag)
      : [...currentTags, tag];
    setIdeaTags(prev => ({ ...prev, [category]: updatedTags }));
  };
        ...
        {tagCategories.map((cat) => (
          <View key={cat.name} style={{ marginBottom: 16 }}>
            ...
            {cat.tags.map((tag,index) => (
                <TagPill key={index} tag={tag} color={cat.color} isSelected={false} onPress={() => toggleTag(cat.name,tag)} removable={false}/>
            ))}
          </View>
        ))}
      </View>
